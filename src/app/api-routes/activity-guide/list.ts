...
import { getActivityGuidesByCongregation, verifyFirebaseSessionCookie } from '@/shared/api/index.server';

export const listActivityGuidesHandler: APIRoute = async ({ url, cookies }) => {
  try {
    ...
    const list = await getActivityGuidesByCongregation(congregationId);
