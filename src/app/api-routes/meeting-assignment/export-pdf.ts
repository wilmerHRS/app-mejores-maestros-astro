...
export const exportMeetingAssignmentsPdfHandler: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const body = await request.json() as { congregationId?: string; weekIds?: string[]; layout?: 'landscape-8' | 'portrait-4' };
    ...
    const layout = body.layout || 'landscape-8';
    const browser = await puppeteer.launch(env.BROWSER);
    try {
      const images = await ensureAssignmentImages(browser, assignments);
      const pdfPage = await browser.newPage();
      await pdfPage.setContent(createPdfHtml(images, layout), { waitUntil: 'networkidle0' });
      
      const pdfOptions = layout === 'portrait-4'
        ? { format: 'A4' as const, landscape: false, printBackground: true, margin: { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' } }
        : { format: 'A4' as const, landscape: true, printBackground: true, margin: { top: '0mm', right: '0mm', bottom: '0mm', left: '0mm' } };

      const pdf = await pdfPage.pdf(pdfOptions);
...
async function ensureAssignmentImages(browser: any, assignments: ExportAssignment[]): Promise<string[]> {
  ...
  const imageData: string[] = new Array(assignments.length);

  for (const assignment of assignments) {
    const meetingKey = `${assignment.congregationId}_${assignment.weekId}`;
    if (!meetingAssignments.has(meetingKey)) {
      ...
      meetingAssignments.set(meetingKey, loadedAssignment);
    }
  }
  ...
  for (let i = 0; i < assignments.length; i += BATCH_SIZE) {
    ...
    await Promise.all(
      batch.map(async (assignment, batchIdx) => {
        ...
        const meetingKey = `${assignment.congregationId}_${assignment.weekId}`;
        const meetingAssignment = meetingAssignments.get(meetingKey)!;
        ...
        imageData[globalIdx] = `data:image/png;base64,${arrayBufferToBase64(await image.arrayBuffer())}`;
      })
    );
  }
...
function createPdfHtml(images: string[], layout: 'landscape-8' | 'portrait-4'): string {
  const pages: string[] = [];

  if (layout === 'portrait-4') {
    const paddedImages = [...images];
    while (paddedImages.length % 4 !== 0) paddedImages.push('');

    for (let index = 0; index < paddedImages.length; index += 4) {
      const pageImages = paddedImages.slice(index, index + 4).map((image) => {
        if (!image) return `<div class="assignment"></div>`;
        return `<div class="assignment"><img src="${image}" /></div>`;
      }).join('');
      pages.push(`<section class="page"><div class="cut-lines"><span class="vertical center"></span><span class="horizontal center"></span></div>${pageImages}</section>`);
    }
    return `<html><head><style>@page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{width:210mm;height:297mm;margin:0;padding:0}body{font-family:Arial,sans-serif}.page{position:relative;width:210mm;height:297mm;display:flex;flex-wrap:wrap;justify-content:center;align-content:center;page-break-after:always}.assignment{width:50%;height:50%;overflow:hidden;background:white;padding:8mm;display:flex;align-items:center;justify-content:center}.assignment img{display:block;width:100%;height:100%;object-fit:contain}.cut-lines{position:absolute;inset:0;z-index:2;pointer-events:none}.cut-lines span{position:absolute;border-color:#64748b;border-style:dashed;border-width:0}.cut-lines .vertical{top:0;bottom:0;border-left-width:.25mm}.cut-lines .vertical.center{left:50%}.cut-lines .horizontal{left:0;right:0;top:50%;border-top-width:.25mm}</style></head><body>${pages.join('')}</body></html>`;
  } else {
    const paddedImages = [...images];
    while (paddedImages.length % 8 !== 0) paddedImages.push('');

    for (let index = 0; index < paddedImages.length; index += 8) {
      const pageImages = paddedImages.slice(index, index + 8).map((image) => {
        if (!image) return `<div class="assignment"></div>`;
        return `<div class="assignment"><img src="${image}" /></div>`;
      }).join('');
