...
import { updateBrotherClient, type Brother, type Group } from '@/shared/api';
import { createBrotherSchema, type CreateBrotherFields } from '../model/brother';

interface EditBrotherModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function EditBrotherModal({
  isOpen,
  onClose,
  brother,
  groups,
  onSuccess
}: EditBrotherModalProps) {
  const [generalError, setGeneralError] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateBrotherFields>({
  ...
  useEffect(() => {
    if (isOpen && brother) {
      ...
      reset({
        names: brother.names,
        paternalLastname: brother.paternalLastname,
        maternalLastname: brother.maternalLastname || '',
        phone: brother.phone,
        gender: brother.gender,
        ageGroup: brother.ageGroup,
        isSickOrDisabled: brother.isSickOrDisabled,
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden relative z-10 shadow-2xl animate-fade-in text-slate-800">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
          <h3 className="text-lg font-bold text-slate-800">Editar Hermano</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        {/* Form */}
        <form onSubmit={handleSubmit(onSubmit)} className="flex-grow overflow-y-auto p-6 space-y-5 max-h-[calc(90vh-5rem)]">
          {generalError && (
          ...
          <div className="space-y-1.5">
            ...
            />
            {errors.names && <p className="text-xs font-semibold text-red-500">{errors.names.message}</p>}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              />
              {errors.paternalLastname && <p className="text-xs font-semibold text-red-500">{errors.paternalLastname.message}</p>}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.maternalLastname && <p className="text-xs font-semibold text-red-500">{errors.maternalLastname.message}</p>}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            {errors.ageGroup && <p className="text-xs font-semibold text-red-500">{errors.ageGroup.message}</p>}
          </div>
          ...
          <div className="py-1">
            <label className="flex items-center gap-3 text-sm font-medium text-slate-700 cursor-pointer select-none">
              ...
              />
              <div>
                <span className="block font-semibold">Enfermo o con Discapacidad</span>
                <span className="block text-[10px] text-slate-400 font-semibold">Tiene limitaciones de salud o movilidad reducida</span>
              </div>
            </label>
          </div>
